...
d     print *,'entering ne_iri_ps_trough_eq:',al,amlt,akp,itime
c
c  We don't do inside the earth; we're just not that kind of model, humph!
	if(al.le.1.0) then
	  ne_iri_ps_trough_eq=0.0
d       print *,'leaving ne_iri_ps_trough_eq early',ne_iri_ps_trough_eq
	  return
	end if
c
c
...
d     print *,'from pp_profile:',pp_factor,al,amlt,akp,a8

	ps_inner=ne_inner_ps(al,amlt,itime,am1,b1,x234)*1.0e6
d     print *,'from ne_inner_ps:',ps_inner,al,amlt,am1,b1,x234
...
d     print *,'values 1:',a8,am1,b1

	  ps_bridge=ano*aheight**(-alpha)
d       print *,'bridge-ps n:',ps_inner,ps_bridge
        off=0.0
        transwidth=0.02
...
d     print *,'entering ne_eq_trough:',swtch1,aheight,transh
          ne_eq_trough_den=ne_eq_trough(al,amlt,akp)
d     print *,'entering check_crossing:',ne_eq_trough_den
...
d          print *,'offset:',akp,a8,zl,(a8-zl),offset
...
d	print *,'** ',swtch1,swtch2,swtch3,swtch4,swtch5
d	print *,al,rintercept,off,aheight,transh,pp_factor
d	print *,outf(1,1),ps_bridge,ps_inner,ne_eq_trough_den
d     print *,ne_iri_ps_trough_eq
c
